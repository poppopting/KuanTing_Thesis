...
def check_ord_gradient(model, train, reg_dict):
    ...
    for i, j in zip(users, items) :
        ...
        update_x[:, rated-1] += (-np.sqrt(1/len(rated)) * model.Q[:, j-1] * diff[idx])[:, np.newaxis]

        idx +=1

    unique_xj = np.unique(xjss)
    update_x = update_x / data_len - reg_dict['reg_xi'] * update_x

    return bu_grad, bi_grad, p_grad, q_grad, t_grad, update_x, (beta_gradient[0], beta_gradient[1], beta_gradient[2])